...
# Allow Flutter frontend to call this API
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
@app.post("/run-json")
def run_pipeline_json(user_text: str = Form(...)):
    """JSON endpoint for Flutter frontend"""
    ...
    # Build AI summary
    items = [r.get("product", "") for r in df_selected.to_dict(orient="records")]
    ai_summary = f"Best picks for {', '.join(items)} selected by Gemini AI. Total: RM {total:.2f}."
    if extracted["constraints"].get("budget"):
        budget = float(extracted["constraints"]["budget"])
        remaining = budget - total
        if remaining >= 0:
            ai_summary += f" Within your budget of RM {budget:.2f} with RM {remaining:.2f} to spare."
        else:
            ai_summary += f" This exceeds your budget of RM {budget:.2f} by RM {abs(remaining):.2f}."
